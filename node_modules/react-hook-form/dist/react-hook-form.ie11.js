...
function useFieldArray(_a) {
    ...
    var _b = control || methods.control, resetFieldArrayFunctionRef = _b.resetFieldArrayFunctionRef, fieldArrayNamesRef = _b.fieldArrayNamesRef, fieldsRef = _b.fieldsRef, defaultValuesRef = _b.defaultValuesRef, unregister = _b.unregister, isDirtyRef = _b.isDirtyRef, readFormStateRef = _b.readFormStateRef;
    ...
    var getFieldValuesByName = function (fields, name) {
        return transformToNestObject(getFieldsValues(fields))[name];
    };
    ...
    var remove = function (index) {
        var updatedFields = removeArrayAt(getFieldValuesByName(fieldsRef.current, name), index);
        resetFields(updatedFields);
        setField(mapIds(updatedFields));
    };
    var insert = function (index, value) {
        var fieldValues = getFieldValuesByName(fieldsRef.current, name);
        resetFields();
        setField(mapIds(__spread(fieldValues.slice(0, index), [
            appendId(value)
...
var Controller = function (_a) {
    var _b, _c, _d, _e;
    var name = _a.name, rules = _a.rules, InnerComponent = _a.as, onChange = _a.onChange, _f = _a.onChangeName, onChangeName = _f === void 0 ? VALIDATION_MODE.onChange : _f, _g = _a.onBlurName, onBlurName = _g === void 0 ? VALIDATION_MODE.onBlur : _g, valueName = _a.valueName, defaultValue = _a.defaultValue, control = _a.control, rest = __rest(_a, ["name", "rules", "as", "onChange", "onChangeName", "onBlurName", "valueName", "defaultValue", "control"]);
    var methods = useFormContext();
    var _h = control || methods.control, defaultValuesRef = _h.defaultValuesRef, setValue = _h.setValue, register = _h.register, unregister = _h.unregister, errors = _h.errors, triggerValidation = _h.triggerValidation, _j = _h.mode, isOnSubmit = _j.isOnSubmit, isOnBlur = _j.isOnBlur, _k = _h.reValidateMode, isReValidateOnBlur = _k.isReValidateOnBlur, isReValidateOnSubmit = _k.isReValidateOnSubmit, isSubmitted = _h.formState.isSubmitted, fieldsRef = _h.fieldsRef, fieldArrayNamesRef = _h.fieldArrayNamesRef;
    ...
    var props = __assign(__assign(__assign(__assign({ name: name }, rest), (onChange
        ? (_b = {}, _b[onChangeName] = eventWrapper(onChange), _b) : (_c = {}, _c[onChangeName] = handleChange, _c))), (isOnBlur || isReValidateOnBlur
        ? (_d = {}, _d[onBlurName] = function () { return triggerValidation(name); }, _d) : {})), (_e = {}, _e[valueName || (isCheckboxInput ? 'checked' : VALUE)] = value, _e));
